...
def add_derived_cfg_before_init(cfg: DictConfig) -> None:
    ...
    # add `datamodule_type` 
    # - Possible values: `NrSpan`, `SrcEvt`
    model_type = cfg.model._target_.split('.')[-1].replace('Model', '')
    cfg.model.model_type = model_type

    # if we're training a NR-span model:
    if model_type == 'NrSpan':
        # add n_unique_labels
        if cfg.datamodule.coarse:
            if cfg.datamodule.use_biolu:
                cfg.datamodule.n_unique_labels = 49  # C[span]=(C[class]-1)*4+1
            else:
                cfg.datamodule.n_unique_labels = 25  # C[span]=(C[class]-1)*2+1

        else:
            if cfg.datamodule.use_biolu:
                cfg.datamodule.n_unique_labels = 97  # C[span]=(C[class]-1)*4+1
            else:
                cfg.datamodule.n_unique_labels = 49  # C[span]=(C[class]-1)*2+1

        # add pretrained_model to cfg.datamodule
        cfg.datamodule.pretrained_model = cfg.model.pretrained_model

    elif model_type == 'NrClass':
        # add pretrained_model to cfg.datamodule
        cfg.datamodule.pretrained_model = cfg.model.pretrained_model

        # add num unique labels, depending on coarse or not
        if cfg.datamodule.coarse:
            cfg.datamodule.n_unique_labels = 13
        else:
            cfg.datamodule.n_unique_labels = 25
...
def load_tx_df(tx_df_name, tasks, data_dir, **kwargs):
    ...
    '''
    # Load from feather
    tx_df = feather.read_feather(f'{data_dir}/{tx_df_name}.feather')
...
def load_split_df(window_size, data_dir, **kwargs):
...
def load_text_df(text_df_name, type_tokens, data_dir, **kwargs):
    ...
    type_token:
        ...
        3: A
    '''
    text_df = feather.read_feather(f'{data_dir}/{text_df_name}.feather', columns=['transcriptid', 'sentenceid', 'type_token'])
...
def load_tid_from_to_pair(DATA_DIR):
    ...
    '''
    pair = feather.read_feather(f'{DATA_DIR}/tid_from_to_pair.feather')
